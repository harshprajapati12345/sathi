...
$pageTitle = 'Subcast';
$adminCurrent = 'master-subcast';

require __DIR__ . '/includes/head.php';

$db = sathi_db();
$subcasts = [];
$res = $db->query("SELECT id, name, status, created_at FROM subcasts ORDER BY name");
if ($res) {
...
$pmHeroTitle = 'Subcast';
$pmHeroLead = 'Manage subcasts (Community-Based Identity Verification).';
...
<section class="admin-page-placeholder">
  <div class="admin-glass-card admin-page-hero admin-page-hero-top">
    ...
    <h2>Subcast Management</h2>
    <p class="lead"><?php echo htmlspecialchars($pmHeroLead); ?></p>
  </div>

  <div class="admin-static-toolbar">
    <button type="button" class="admin-btn admin-btn-primary" onclick="showAddModal()">Add Subcast</button>
    <input type="search" class="admin-input-search" placeholder="Search subcasts..." style="max-width:320px;" oninput="filterTable(this.value)">
  </div>
...
<div id="dataModal" class="admin-modal-overlay" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:9999; align-items:center; justify-content:center;">
  <div class="admin-glass-card" style="width:100%; max-width:400px; padding:24px;">
    <h3 id="modalTitle">Add Subcast</h3>
...
function showAddModal() {
    currentAction = 'add';
    document.getElementById('modalTitle').textContent = 'Add Subcast';
...
function editModal(row) {
    currentAction = 'edit';
    document.getElementById('modalTitle').textContent = 'Edit Subcast';
...
function deleteRow(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const body = new URLSearchParams({ table: 'subcasts', action: 'delete', id: id });
            fetch('custom-master-action.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: body.toString()
