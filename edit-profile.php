...
$stmtC = $db->prepare("SELECT * FROM candidates WHERE email = ? OR mobile_number = ?");
$stmtC->bind_param("ss", $user['email'], $user['mobile']);
$stmtC->execute();
$candidate = $stmtC->get_result()->fetch_assoc();
if (!$candidate) $candidate = [];

$data = array_merge($user, $candidate);

// Map some fields back to form names
// Map some fields back to form names
$data['digamber'] = $data['digamber'] ?? 'yes'; // Default
$data['addr_perm'] = $data['permanent_address'] ?? '';
$data['addr_curr'] = $data['candidate_current_address'] ?? '';
$data['complexion'] = $data['candidate_complexion'] ?? ''; // Pincode
$data['education'] = $data['higher_education'] ?? '';
$data['annual_income'] = $data['candidate_annual_income'] ?? '';
$data['marital_status'] = $data['widow_divorce'] ?? '';
$data['blood_group'] = $data['handicapped_physical_deficiency'] ?? ''; // Handicapped
$data['languages'] = $data['language_known'] ?? '';
$data['occupation'] = $data['candidate_occupation'] ?? '';
$data['job_firm'] = $data['occupation_firm'] ?? '';
$data['job_post'] = $data['occupation_designation'] ?? '';

// Hobbies logic -> checkbox
$hobbies = explode(', ', $data['hobbies'] ?? '');
foreach($hobbies as $h) {
    if(trim($h)) $data['hobby[]'][] = trim($h);
}

// Remove sensitive
unset($data['password_hash']);

$pageTitle = 'My Profile ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Shadikibaat | Marriage Bureau Onboarding';
    ...
    <div class="container reg-shell reg-shell--hero">
        <div class="reg-form-column">
            <div class="reg-card">
                ...
                <form id="profileForm" class="reg-form" action="api/update-profile.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="profile-section">
                        ...
                        <div class="reg-grid">
                            ...
                            <!-- 2. Are You Digambar Jain -->
                            <div<?php echo sathi_reg_field_wrap_attrs('digamber_jain'); ?>>
                                <div class="reg-float reg-float-select">
                                    <select id="digamber" name="digamber" <?php echo sathi_reg_field_required_attr('digamber_jain'); ?> style="pointer-events: none; background-color: #f9f9f9; color: #888;" tabindex="-1">
                                        <option value="yes" selected>Yes</option>
                                        <option value="no">No</option>
                                    </select>
                                    <label for="digamber">Are You Digambar Jain ? (परिचय सम्मेलन सिर्फ दिगम्बर जैन के लिये है)</label>
                                </div>
                            </div>
                                ...
                                <div class="reg-grid" style="margin-bottom: 15px;">
                                    ...
                                    <div class="reg-float reg-float-select">
                                        <select id="mandir" name="mandir" required>
                                            <?php sathi_reg_render_select_options($masters['mandir'] ?? [], $data['mandir_id'] ?? ''); ?>
                                        </select>
                                    ...
                                    <div class="reg-float reg-float-select">
                                        <select id="subcast" name="subcast" required>
                                            <?php sathi_reg_render_select_options($masters['subcast'] ?? [], $data['subcast_id'] ?? ''); ?>
                                        </select>
                                    ...
                                    <div class="reg-float reg-float-select">
                                        <select id="gotra" name="gotra" required>
                                            <?php sathi_reg_render_select_options($masters['gotra'] ?? [], $data['gotra_id'] ?? ''); ?>
                                        </select>
                                ...
                                <div class="reg-split" style="gap: 15px; margin-bottom: 15px;">
                                    <div class="reg-subcard" style="padding: 12px; background: #fff; border: 1px solid #fce7f3; box-shadow: none;">
                                        ...
                                        <div class="reg-float" style="margin-bottom: 10px;">
                                            <input type="text" id="ref1_name" name="ref1_name" placeholder=" " value="<?php echo htmlspecialchars($data['reference_person_1_name'] ?? '', ENT_QUOTES); ?>" required>
                                            <label for="ref1_name">Name / नाम / નામ <span style="color:red;">*</span></label>
                                        </div>
                                        <div class="reg-float" style="margin-bottom: 10px;">
                                            <input type="tel" id="ref1_mobile" name="ref1_mobile" placeholder=" " value="<?php echo htmlspecialchars($data['reference_person_1_mobile'] ?? '', ENT_QUOTES); ?>" required maxlength="10" pattern="[0-9]{10}" inputmode="numeric">
                                        ...
                                        <div class="reg-float">
                                            <input type="text" id="ref1_relation" name="ref1_relation" placeholder=" " value="<?php echo htmlspecialchars($data['reference_person_1_relation'] ?? '', ENT_QUOTES); ?>">
                                    ...
                                    <div class="reg-subcard" style="padding: 12px; background: #fff; border: 1px solid #fce7f3; box-shadow: none;">
                                        ...
                                        <div class="reg-float" style="margin-bottom: 10px;">
                                            <input type="text" id="ref2_name" name="ref2_name" placeholder=" " value="<?php echo htmlspecialchars($data['reference_person_2_name'] ?? '', ENT_QUOTES); ?>" required>
                                            <label for="ref2_name">Name / नाम / નામ <span style="color:red;">*</span></label>
                                        </div>
                                        <div class="reg-float" style="margin-bottom: 10px;">
                                            <input type="tel" id="ref2_mobile" name="ref2_mobile" placeholder=" " value="<?php echo htmlspecialchars($data['reference_person_2_mobile'] ?? '', ENT_QUOTES); ?>" required maxlength="10" pattern="[0-9]{10}" inputmode="numeric">
                                            <label for="ref2_mobile">Mobile Number / मोबाइल नंबर / મોબાઇલ નંબર <span style="color:red;">*</span></label>
                                            <span class="reg-error-msg">Enter 10-digit mobile number / 10 अंकों का मोबाइल नंबर दर्ज करें / 10 અંકનો મોબાઇલ નંબર દાખલ કરો</span>
                                        </div>
                                        <div class="reg-float">
                                            <input type="text" id="ref2_relation" name="ref2_relation" placeholder=" " value="<?php echo htmlspecialchars($data['reference_person_2_relation'] ?? '', ENT_QUOTES); ?>">
                    ...
                    <div class="profile-section" style="margin-top: 40px; border-top: 1px solid #eee; padding-top: 40px;">
                ...
                <div class="profile-section" style="padding: 20px; background: #fff5f5; border: 1px solid #fed7d7; border-radius: 8px;">
                    <h2 class="reg-section-title" style="color: #e53e3e; margin-bottom: 10px;">Profile Deactivation Request</h2>
                    <p style="font-size: 14px; color: #718096; margin-bottom: 20px;">If you wish to deactivate your profile from the website, please submit a request below. Our admin will review and process your request.</p>
                    
                    <form id="deactivateForm" class="reg-form">
                        <div class="reg-float reg-float-select" style="margin-bottom: 15px;">
                            <select id="deactivate_reason" name="reason" required>
                                <option value="" disabled selected></option>
                            ...
                            <label for="deactivate_reason">Reason for deactivation <span style="color:red;">*</span></label>
                        </div>
                        <button type="submit" class="reg-btn" style="background-color: #e53e3e; color: white; border: none; padding: 10px 20px; cursor: pointer; border-radius: 5px;">Request Deactivation</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    (function () {
        ...
        // Persistence logic
        function saveDraft() {
            // Disabled in edit profile
        }

        async function loadDraft() {
            const data = <?php echo json_encode($data); ?>;
            try {
                // Map specific fields like hobbies
                if (data.hobbies) {
                    const hobbiesArray = data.hobbies.split(',').map(h => h.trim().toLowerCase());
                    document.querySelectorAll('input[name="hobby[]"]').forEach(cb => {
                        if (hobbiesArray.includes(cb.value.toLowerCase())) {
                            cb.checked = true;
                        }
                    });
                }
    ...
    if (deactivateForm) {
        deactivateForm.addEventListener('submit', function(e) {
            ...
            fetch('api/request-deactivation.php', {
                method: 'POST',
                body: fd
            })
            .then(r => r.json())
            .then(data => {
                if (data.ok) {
                    Swal.fire({
                        ...
                        text: 'Your profile deactivation request has been sent to the admin.',
                        confirmButtonColor: '#e94e77'
                    });
                ...
                btn.disabled = false;
                btn.textContent = 'Request Deactivation';
            })
            .catch(err => {
                console.error(err);
                Swal.fire({
